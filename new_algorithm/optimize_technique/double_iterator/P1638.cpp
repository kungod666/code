...
vector<int> hash_set(2005);
vector<int> stand(2005);
vector<pair<int,int>> ordi;
int ans = INT_MAX;
int main(){
    cin >> n >> m;
    for(int i = 1;i <= n;i++){
    ...
    int left,right;
    left = 1;
    hash_set[painting[left]] = 1;
    int count = 1;
    for(right = 2;right <= n;right++){
        if(hash_set[painting[right]] == 0){
            count++;
        }
        hash_set[painting[right]]++;
        while(hash_set[painting[left]] > 1){
            hash_set[painting[left]]--;
            left++;
        }
        if(count == m){
