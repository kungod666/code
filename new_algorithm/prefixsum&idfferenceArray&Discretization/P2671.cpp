...
ll color[100005];
ll group[100005][2];
ll sum1[100005][2]; //i+j+m
ll sum2[100005][2];//n[i]+n[j]+n[m]
ll sum3[100005][2];//mn[m]+in[i]+jn[j]
int main(){
    cin >> n >> m;
    for(int i = 1;i <= n;i++){
        cin >> num[i];
    ...
    for(int i = 1;i <= n;i++){
        group[color[i]][i%2]++;
        sum1[color[i]][i%2] += i;
        sum3[color[i]][i%2] += i*num[i];
        sum2[color[i]][i%2] += num[i];
    }
    ll ans = 0;
    for(int i = 1;i <= m;i++){
        if(group[i][1] > 1){
            ans += (group[i][1]-2)*sum3[i][1] + sum2[i][1]*sum1[i][1];
        }
        if(group[i][0] > 1){
            ans += (group[i][0]-2)*sum3[i][0] + sum2[i][0]*sum1[i][0];
        }
    }
    ans %= 10007;
    cout << ans; 
