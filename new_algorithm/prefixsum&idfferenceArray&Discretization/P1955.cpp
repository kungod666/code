...
ll t;
int find(int x,vector<ll>& ysnb){
    if(ysnb[x] != x){
        ysnb[x] = find(ysnb[x],ysnb);
        //路径压缩，直接挂根上
    }
    return ysnb[x];
}
void unite(int x,int y,vector<ll>& ysnb){
    int rootx = find(x,ysnb);
    int rooty = find(y,ysnb);
...
int main(){
    ...
    while(t--){
        ...
        for(int i = 0;i < n;i++){
            ...
            //无论是哪种条件，都应该把a，b放入用于离散化的集合mmp，防止漏元素
            //否则后续用lower_bound再去访问后会发生数组越界
            mmp.push_back(a);
            mmp.push_back(b);
        }
        sort(mmp.begin(),mmp.end());
        auto m = unique(mmp.begin(),mmp.end());
        mmp.erase(m,mmp.end());
        int counts = equal1.size();
        vector<ll> ysnb(mmp.size()+5);
        for(int i = 0;i < ysnb.size();i++){
            ysnb[i] = i;
        }
        int times = unequal.size();
        for(int i = 0;i < counts;i++){
            int wcnm = lower_bound(mmp.begin(),mmp.end(),equal1[i]) - mmp.begin();
            int cnm = lower_bound(mmp.begin(),mmp.end(),equal2[i])-mmp.begin();
            /*if(ysnb[wcnm] != -1){
                ysnb[cnm] = ysnb[wcnm];
            }
            else if(ysnb[cnm] != -1){
        ...
        for(int i = 0;i < times;i++){
            int zzj = lower_bound(mmp.begin(),mmp.end(),unequal[i].first)-mmp.begin();
            int whx = lower_bound(mmp.begin(),mmp.end(),unequal[i].second)-mmp.begin();
            if(find(zzj,ysnb) == find(whx,ysnb)){
                flag = 0;
                break;
            }
